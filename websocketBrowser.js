 ...
 * @param func 映射方法
 */
export function addNoticeFunction(func){
    noticeFuncMap[func.name]=func
}
/**
 * 获取当前关闭连接类型
 * @returns （客户端登出|服务端踢出）
 ...
 */
export function socket(url, data, file) {
    return new Promise(async resolve => {
        ...
        const flag = watch(dataBox, () => {
            if (dataBox.value.count === count) {
                resolve(dataBox.value)
                flag()
            }
        })
    })
...
function waitClient() {
    return new Promise((resolve) => {
        ...
        const flag = watch(() => isClient.value, (client) => {
            if (client) {
                resolve()
                flag()
            }
...
/**
 * 新建websocket连接
 * @param _token : String 唯一token码（可以为空）
 * @param _path 指定路径 （不为空时代替setWebsocketPath方法指定的路径）
 * @returns Promise<null> 异常捕获
 */
export function newSocket(_token, _path) {
